// Auto upload handler - checks for uploaded files and shows QR codes
console.log('[AUTO UPLOAD] Handler starting...');
...
async function checkForUploads() {
    try {
        ...
        const response = await fetch('http://localhost:8888/latest_upload.json?' + Date.now());
        if (response.ok) {
            ...
            if (data.timestamp !== lastUploadCheck) {
                ...
                console.log('[AUTO UPLOAD] New upload detected:', data.link);
...
window.fetch = function(...args) {
    ...
    if (url.includes('drum.usercontent.google.com/download/') || 
        ...
        console.log('[AUTO UPLOAD] Potential download detected:', url);
...
XMLHttpRequest.prototype.open = function(method, url, ...rest) {
    if (url && (url.includes('/download/') || url.includes('/generate'))) {
        console.log('[AUTO UPLOAD] XHR download detected:', url);
...
URL.createObjectURL = function(blob) {
    if (blob && blob.type && blob.type.startsWith('video/')) {
        console.log('[AUTO UPLOAD] Video blob created, size:', blob.size);
...
if (window.location.href.includes('labs.google')) {
    console.log('[AUTO UPLOAD] On Google Labs page, starting monitor...');
    startDownloadMonitor();
}

console.log('[AUTO UPLOAD] Handler ready - will detect downloads and show upload results automatically');